...
Low-resolution source in `scripts/download-foulborn-art.mjs`:

- GGG's documented Public Stash API
  - uses the official mutated CDN URLs from `item.icon`
  - requires OAuth `service:psapi` access plus an identifiable `User-Agent`
  - only sees currently public listings, so it is not guaranteed to surface the full Foulborn catalog in one pass
...
1. run `node scripts/download-foulborn-art.mjs --start-id <next_change_id>`
...
Keep the printed `next_change_id` so later refreshes can resume from the last observed point in the official stash stream.

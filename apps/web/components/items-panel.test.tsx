...
describe("ItemsPanel", () => {
  ...
  it("copies occupied desktop item text to the clipboard when clicking a gear slot", async () => {
    ...
    await waitFor(() => expect(container.querySelector(".item-copy-toast")?.textContent).toContain("Item copied to clipboard"));
    expect(container.querySelector(".gear-slot--amulet")?.className).toContain("gear-slot--copyable");
  });
  ...
  it("orders socketed jewels by category instead of passive tree socket order", () => {
    const payload: BuildPayload = {
      ...buildViewerPayloadFixture,
      activeItemSetId: 1,
      itemSets: [
      ...
      items: [
        ...
        {
          anointments: [],
          base: "Glimmering Ghastly Eye Jewel",
          corrupted: false,
          crafted: [],
          enchantments: [],
          explicits: [],
          ...
          influences: [],
          mirrored: false,
          name: "Glimmering Ghastly Eye Jewel",
          rarity: "Magic",
          raw: "Rarity: Magic\nGlimmering Ghastly Eye Jewel",
          scourgedMods: [],
          crucibleMods: [],
          synthesizedMods: [],
        },
      ...
      treeSpecs: [
        {
          ...
          nodes: [...buildViewerPayloadFixture.treeSpecs[0].nodes],
          sockets: [
            { itemId: 4104, nodeId: 600 },
            { itemId: 4101, nodeId: 100 },
            { itemId: 4112, nodeId: 200 },
            { itemId: 4106, nodeId: 300 },
            { itemId: 4105, nodeId: 400 },
            { itemId: 4103, nodeId: 500 },
            { itemId: 4107, nodeId: 700 },
            { itemId: 4108, nodeId: 800 },
            { itemId: 4110, nodeId: 900 },
            { itemId: 4102, nodeId: 1000 },
            { itemId: 4109, nodeId: 1100 },
            { itemId: 4111, nodeId: 1200 },
          ],
        },
      ],
    };
    ...
    const { container } = render(<ItemsPanel payload={payload} treeIndex={0} />);
    const jewelOrder = Array.from(container.querySelectorAll(".gear-jewel-row img")).map((image) => image.getAttribute("alt"));

    expect(jewelOrder).toEqual([
      "Voices",
      "Atlas Shard",
      "Bramble Core",
      "Megalomaniac",
